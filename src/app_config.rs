...
pub struct Config {
    pub mqtt: MqttConfig,
    pub device: MusicCastConfig,
}

pub fn read_config() -> Result<(MqttConfig, MusicCastConfig)> {
    ...
    let musiccast_config = config.device;

    Ok((mqtt_config, musiccast_config))
}
